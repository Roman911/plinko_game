<script lang="ts">
  import { binColorsByRowCount, binColorsByRowCount2, binPayouts } from '$lib/constants/game';
  import { plinkoEngine2, riskLevel2, rowCount2, winRecords2 } from '$lib/stores/game';
  ...
  $: {
    if ($winRecords2.length) {
      const lastWinBinIndex = $winRecords2[$winRecords2.length - 1].binIndex;
...
<div class="flex h-[clamp(10px,0.352px+2.609vw,16px)] w-full justify-center lg:h-7">
  {#if $plinkoEngine2}
    <div class="flex gap-[1%]" style:width={`${($plinkoEngine2.binsWidthPercentage ?? 0) * 100}%`}>
      {#each binPayouts[$rowCount2][$riskLevel2] as payout, binIndex}
        <!-- Font-size clamping:
        ...
        <div
          ...
          style:background-color={binColorsByRowCount2[$rowCount2].background[binIndex]}
          style:--shadow-color={binColorsByRowCount2[$rowCount2].shadow[binIndex]}
        >
          {payout}{payout < 100 ? '×' : ''}
        </div>
      {/each}
