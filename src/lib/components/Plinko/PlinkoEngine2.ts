...
class PlinkoEngine {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.rowCount = get(rowCount2);
    this.riskLevel = get(riskLevel2);
    betAmount.subscribe((value) => (this.betAmount = value));
    rowCount2.subscribe((value) => this.updateRowCount(value));
    riskLevel2.subscribe((value) => (this.riskLevel = value));
    ...
    this.render = Matter.Render.create({
      ...
      canvas: this.canvas,
      options: {
        width: PlinkoEngine.WIDTH2,
        height: PlinkoEngine.HEIGHT2,
        background: '#0f1728',
        wireframes: false,
      },
    });
  ...
  get binsWidthPercentage(): number {
    ...
    return (lastPinX - this.pinsLastRowXCoords[0]) / PlinkoEngine.WIDTH2;
  }
  ...
  private get pinDistanceX(): number {
    const lastRowPinCount = 7 + this.rowCount - 1;
  ...
  private placePinsAndWalls() {
    ...
    for (let row = 0; row < this.rowCount; ++row) {
      ...
      for (let col = 0; col < 7 + row; ++col) {
        const colX = rowPaddingX + ((this.canvas.width - rowPaddingX * 2) / (7 + row - 1)) * col;
